const state={
  ...
  drivers:['Jordan Lee','Sam Rivera','Morgan Tate'],
  locations:['Riverbend Grain Co.','Heartland Processing','Midwest Ethanol']
};
...
function renderDrivers(){const select=$('#demoHauler');const selected=select.value;select.innerHTML=`<option value="">Choose Hauled By</option>${state.drivers.map(name=>`<option>${name}</option>`).join('')}<option value="Other">Other</option>`;if([...select.options].some(option=>option.value===selected))select.value=selected;$('#driverList').innerHTML=state.drivers.map((name,i)=>`<div class="data-chip"><span>${name}</span><button type="button" data-remove-driver="${i}">Remove</button></div>`).join('')}
function renderLocations(){const select=$('#demoDeliveredTo');const selected=select.value;select.innerHTML=`<option value="">Choose Delivered To</option>${state.locations.map(name=>`<option>${name}</option>`).join('')}<option value="Other">Other</option>`;if([...select.options].some(option=>option.value===selected))select.value=selected;$('#locationList').innerHTML=state.locations.map((name,i)=>`<div class="data-chip"><span>${name}</span><button type="button" data-remove-location="${i}">Remove</button></div>`).join('')}
...
function extract(){const f=$('#ticketForm');f.ticket_number.value='RB-10482';f.date.value='06/21/2026';f.crop.value='Corn';f.hauled_from.value='North 48';f.delivered_to.value='Riverbend Grain Co.';$('#otherDestination').hidden=true;f.other_destination.value='';f.bushels.value='2811.43';f.hauled_by.value='Jordan Lee';f.moisture.value='15.2';$('#scanStatus').textContent='Extraction complete. Review highlighted fields before submitting.';toast('Ticket data extracted with AI confidence checks')}
...
$('#ticketForm').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;if(!f.ticket_number.value)return toast('Extract the sample ticket first');const hauler=f.hauled_by.value==='Other'?f.other_hauler.value.trim():f.hauled_by.value;if(!hauler)return toast('Choose or enter a driver');const destination=f.delivered_to.value==='Other'?f.other_destination.value.trim():f.delivered_to.value;if(!destination)return toast('Choose or enter a delivery location');const amount=Number(f.bushels.value),bin=state.bins.find(b=>b.name===f.hauled_from.value);bin.current=Math.max(0,bin.current-amount);const contractId=state.saleType==='Contract'?f.contract_id.value:'';if(state.saleType==='Contract'&&!contractId)return toast('Choose a contract before submitting');const contract=state.contracts.find(c=>c.id===contractId);if(contract)contract.delivered+=amount;state.tickets.unshift({id:Date.now(),number:f.ticket_number.value,date:f.date.value,crop:f.crop.value,from:f.hauled_from.value,to:destination,bushels:amount,assignment:state.saleType,payment:'Not paid',hauler,contract:contractId});renderAll();toast('Ticket submitted. Inventory and contract balances updated.');go('dashboard')};
...
$('#driverForm').onsubmit=e=>{e.preventDefault();const name=e.currentTarget.elements.name.value.trim();if(!name)return;if(!state.drivers.some(driver=>driver.toLowerCase()===name.toLowerCase()))state.drivers.push(name);e.currentTarget.reset();renderDrivers();toast('Driver added to the Hauled By dropdown.')};
$('#driverList').onclick=e=>{if(e.target.dataset.removeDriver!==undefined){state.drivers.splice(Number(e.target.dataset.removeDriver),1);renderDrivers();toast('Driver removed.')}};
$('#locationForm').onsubmit=e=>{e.preventDefault();const name=e.currentTarget.elements.name.value.trim();if(!name)return;if(!state.locations.some(location=>location.toLowerCase()===name.toLowerCase()))state.locations.push(name);e.currentTarget.reset();renderLocations();toast('Location added to the Delivered To dropdown.')};
$('#locationList').onclick=e=>{if(e.target.dataset.removeLocation!==undefined){state.locations.splice(Number(e.target.dataset.removeLocation),1);renderLocations();toast('Location removed.')}};
$('#demoHauler').onchange=e=>{$('#otherHauler').hidden=e.target.value!=='Other';if(e.target.value!=='Other')$('#otherHauler').value=''};
$('#demoDeliveredTo').onchange=e=>{$('#otherDestination').hidden=e.target.value!=='Other';if(e.target.value!=='Other')$('#otherDestination').value=''};
$('#exportButton').onclick=()=>toast('CSV export prepared — disabled from downloading in the public demo.');
function renderAll(){renderDashboard();renderBins('#inventoryBins');renderContracts();renderTickets();renderUsers();populateSelects();renderDrivers();renderLocations()}
