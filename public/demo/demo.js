...
$('#ticketForm').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;if(!f.ticket_number.value)return toast('Extract the sample ticket first');const hauler=f.hauled_by.value==='Other'?f.other_hauler.value.trim():f.hauled_by.value;if(!hauler)return toast('Choose or enter a driver');const amount=Number(f.bushels.value),bin=state.bins.find(b=>b.name===f.hauled_from.value);bin.current=Math.max(0,bin.current-amount);const contractId=state.saleType==='Contract'?f.contract_id.value:'';if(state.saleType==='Contract'&&!contractId)return toast('Choose a contract before submitting');const contract=state.contracts.find(c=>c.id===contractId);if(contract)contract.delivered+=amount;state.tickets.unshift({id:Date.now(),number:f.ticket_number.value,date:f.date.value,crop:f.crop.value,from:f.hauled_from.value,to:f.delivered_to.value,bushels:amount,assignment:state.saleType,payment:'Not paid',hauler,contract:contractId});renderAll();toast('Ticket submitted. Inventory and contract balances updated.');go('dashboard')};
...
function renderAll(){renderDashboard();renderBins('#inventoryBins');renderContracts();renderTickets();renderUsers();populateSelects();renderDrivers()}
